...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val finalResultDf = spark.sql("select rectangle, count(point) as noOfPoints from joinResult group by rectangle order by rectangle asc").persist()
    finalResultDf.createOrReplaceTempView("finalResult")

    return finalResultDf.coalesce(1) // YOU NEED TO CHANGE THIS PART
